...
public class DataLogicIntegration extends BeanFactoryDataSingle {
        ...
        Transaction t = new Transaction(s) {
            public Object transact() throws BasicException {
                ...
                if (new PreparedSentence(s,
                            "UPDATE CUSTOMERS SET SEARCHKEY = ?, NAME = ?, NOTES = ?, ADDRESS=?, ADDRESS2=?, CITY=?, COUNTRY=?, EMAIL=?, FAX=?, FIRSTNAME=?, LASTNAME=?, PHONE=?, PHONE2=?, POSTAL=?, REGION=?, VISIBLE = " + s.DB.TRUE() + " WHERE ID = ?",
                            SerializerWriteParams.INSTANCE
                                ...
                                setString(6, customer.getCity());
                                setString(7, customer.getCountry());
                                setString(8, customer.getEmail());
                                setString(9, customer.getFax());
                                ...
                                setString(14, customer.getPostal());
                                setString(15, customer.getRegion());
                                setString(16, customer.getId());
                            }}) == 0) {

                    // If not updated, try to insert
                    new PreparedSentence(s,
                            "INSERT INTO CUSTOMERS(ID, SEARCHKEY, NAME, NOTES, VISIBLE,ADDRESS, ADDRESS2, CITY, COUNTRY, EMAIL, FAX, FIRSTNAME, LASTNAME, PHONE, PHONE2, POSTAL, REGION) VALUES (?, ?, ?, ?, " + s.DB.TRUE() + ",?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                            SerializerWriteParams.INSTANCE
                            ).exec(new DataParams() { public void writeValues() throws BasicException {
                                ...
                                setString(8, customer.getCountry());
                                setString(9, customer.getEmail());
                                setString(10, customer.getFax());
                                setString(11, customer.getFirstname());
                                setString(12, customer.getLastname());
                                setString(13, customer.getPhone());
                                setString(14, customer.getPhone2());
                                setString(15, customer.getPostal());
                                setString(16, customer.getRegion());
                            }});
                }
    ...
    public List getTickets() throws BasicException {
        return new PreparedSentence(s
                , "SELECT T.ID, T.TICKETTYPE, T.TICKETID, R.DATENEW, R.MONEY, P.ID, P.NAME, C.ID, C.TAXID, C.SEARCHKEY, C.NAME, R.ATTRIBUTES FROM RECEIPTS R JOIN TICKETS T ON R.ID = T.ID LEFT OUTER JOIN PEOPLE P ON T.PERSON = P.ID LEFT OUTER JOIN CUSTOMERS C ON T.CUSTOMER = C.ID WHERE (T.TICKETTYPE = 0 OR T.TICKETTYPE = 1) AND T.STATUS = 0"
                , null
